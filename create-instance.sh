...
if [[ "$InstanceState" == 0 || "$InstanceState" == 16 || "$InstanceState" == 32 || "$InstanceState" == 64 || "$InstanceState" == 80 ]]; then
...
sed -e "s/INSTANCE_NAME/$InstanceName/" -e "s/INSTANCE_IP/$IP/" record1.json >/tmp/record.json
