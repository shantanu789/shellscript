...
INSTANCE_STATE=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=$InstanceName" | jq .Reservations[].Instances[].State.Code | sed -e 's/"//g')

if [ "$(INSTANCE_STATE)" == '16' || "$(INSTANCE_STATE)" == '32' || $(INSTANCE_STATE) == '48' || $(INSTANCE_STATE) == '64' || $(INSTANCE_STATE) == '80' ]; then
  echo -e "\n\e[33m $InstanceName is exists and ${INSTANCE_STATE_CODE[$INSTANCE_STATE]}\n"
  exit 1
fi
